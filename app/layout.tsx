...
export default function RootLayout({
  ...
  const isBlogPage = safePathname.startsWith("/Blog");
  // A blog *detail* page (`/Blog/<slug>`) should keep its own metadata (set via Next.js `export const metadata` or the page itself); we only blanket-override the listing.
  const isBlogDetail = safePathname.startsWith("/Blog/");
  const backgroundImage = isBlogPage ? "/zoyaa-bg.png" : (backgrounds[safePathname] || "/zoyaa-bg.png");
  const currentMetadata = isBlogDetail
    ? null
    : isBlogPage
    ? metadata["/Blog"]
    : (metadata[safePathname] || { title: "Zoya Kou | Full Stack Digital Manager", description: "Full Stack Digital Manager, QA Automation, Technical Writing, Web Development, Digital Marketing, Project Management, Virtual Assistant, Zoya Kou" });

  // Dynamically update title and meta description (skip on blog detail pages)
  useEffect(() => {
  ...
  return (
    <html lang="en">
      <head>
        ...
        />
        {/* Meta tags for SEO */}
        {currentMetadata ? <meta name="description" content={currentMetadata.description} /> : null}
